...
Triangle_ej2::Triangle_ej2()
  : Drawable({ ShaderUnit("vertex_shader_ej2.glsl", GL_VERTEX_SHADER),
...
Triangle_ej2::render()
{
  ...
  float pos_offset = static_cast<float>(cos(timeValue) / 3.0);
  int pos_offset_location =
    glGetUniformLocation(this->shaderProgram.getShaderProgram(), "pos_offset");
  glUniform4f(pos_offset_location, pos_offset, 0.0f, 0.0f, 1.0f);
