...
ShaderProgram::ShaderProgram(std::vector<ShaderUnit> &&shaderUnits) : shaders(std::move(shaderUnits)) {
	GLuint shaderProgram = glCreateProgram();
	for(const ShaderUnit &s : this->shaders) {
		glAttachShader(shaderProgram, s.getShader());
	}
	glLinkProgram(shaderProgram);
	check_status_program(shaderProgram, GL_LINK_STATUS, "ERROR::SHADERPROGRAM::LINK_FAILED\n");
