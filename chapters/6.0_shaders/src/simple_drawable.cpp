...
SimpleDrawable::SimpleDrawable(std::vector<float> &&vertices,
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(VERTICES_TYPE)*this->vertices.size(), this->vertices.data(), GL_STATIC_DRAW);

	glVertexAttribPointer(0, this->numVerts, GL_FLOAT, GL_FALSE, this->numVerts * sizeof(float), (void*)0);
    	...
    	// bind the this->VAO (it was already bound, but just to demonstrate): seeing as we only have a single this->VAO we can 
...
const ShaderProgram& SimpleDrawable::getShaderProgram(){
